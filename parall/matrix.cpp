...
static void get_main_block_parallel (std::pair<double, int> &pr, double *a_buf, int n, int m, int k, int p, int col_cnt, int step, double eps, double *buf, double *buf2, int *r)
{
  int s = n / m;
  pr.first = 1e308;
  pr.second = -1;
  (void)col_cnt;
  for (int i = step + k; i < s; i += p)
    {
      int ret = get_reversed_matrix_for_block (a_buf, m, m, m, i, 0, buf, eps, buf2, r);
      if (ret == SING_MATRIX)
        continue;
      double check = norma_matrix (buf, m, m);
      if (check < pr.first)
	...
	for (int step = 0; step < s + (ost != 0); step++)
	  {
	    ...
	    if (ost != 0 && step == s && owner == k) //если последний столбец, который не поделился
	      {
	    ...
	    else if (!(ost != 0 && step == s))
	      {
	        std::pair <double, int> send, get;
	        get_main_block_parallel (send, a_buf, n, m, k, p, col_cnt, step, eps, buf, buf_mm, r_m);
